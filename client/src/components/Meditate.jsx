...
const Meditate = ({ changeMeditationTab }) => {
  ...
  const [meditateDuration, setMeditateDuration] = useState(
    getHoursMinutesSeconds(600)
  );
  ...
  }, [currentBrightness, incrementFraction, meditateDuration, timerActive]);

  const getDefaultMeditationTime = () =>
    axios.get('/api/meditationTime').then((res) => {
      setUserMeditationTime(Number(res.data.defaultMeditationTime));
      setMeditateDuration(
        getHoursMinutesSeconds(res.data.defaultMeditationTime)
      );
    });

  const setDefaultMeditationTime = (newTime) =>
    axios
  ...
  const startTimer = useCallback(() => {
    ...
    setTimerActive(true);
  }, [currentBrightness, meditateDuration]);
    ...
    <div>
      {journalView ? (
        ...
        <div className="meditationTimer">
          ...
          <div className="timerRow">
            <button disabled={timerRunning} onClick={startTimer}>
              Start
            </button>
            <button onClick={stopTimer}>Stop</button>
            <button onClick={resetTimer}>Reset</button>
            <button disabled={timerRunning} onClick={setDefaultTime}>
              Set Default Time
            </button>
            <button className="info" onClick={displayTimerInfo}>
              <FontAwesomeIcon icon={faInfoCircle} />
            </button>
          </div>

          {defaultTimeChanged && (
            <div id="defaultTimeMsg">Default time changed!</div>
          )}

          {timerInfoShow && (
            <div className="infoText">
              <p>
                Here you can record a meditation session. A gong will sound both
                when the session starts and when it ends. Press{' '}
                <span className="textSpan">Start</span> to start the timer,{' '}
                <span className="textSpan">Stop</span> to stop it,{' '}
                <span className="textSpan">Reset</span> to go back to the
                beginning, or <span className="textSpan">Set Default Time</span>{' '}
                to personalize your default meditation time. You can adjust the
                time with either the up and down arrows, or you can click on the
                numbers and enter in the appropriate time. You will be prompted
                to make a journal entry after the meditation session ends, where
                you can log your thoughts, or you can cancel to not record a
                journal entry.
              </p>
            </div>
          )}
