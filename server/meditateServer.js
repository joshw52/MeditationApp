...
app.get('/api/meditationTime', requireLogin, function(req, res) {
	try {
		const item = db.collection('users').findOne({
...
app.post('/api/meditationTime', requireLogin, function(req, res) {
	try {
		const item = db.collection('users').findOne({
			username: req.session.username,
		});
		if (item) {
			const updatedItem = db.collection('users').findOneAndUpdate(
			{ _id: item._id },
			{ $set: {
...
app.post('/api/meditationEntry', requireLogin, function(req, res) {
	try {
		...
		db.collection('meditationrecord').insertOne(meditationEntry);
		res.json({
...
app.get('/api/accountInfoLoad', requireLogin, function(req, res) {
	try {
...
app.post('/api/accountModify', requireLogin, function(req, res) {
	try {
...
app.post('/api/accountLoginModify', requireLogin, function(req, res) {
	try {
...
app.get('/api/progress', requireLogin, function(req, res) {
	try {
		const meditationRecords = db.collection('meditationrecord').find({
			$and: [
				{
					username: {
...
app.post('/api/modifyJournalEntry', requireLogin, function(req, res) {
	try {
...
app.post('/api/deleteJournalEntry', requireLogin, function(req, res) {
	try {
